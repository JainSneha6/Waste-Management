...
export default function ImageUpload() {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/predict/typeofwaste', formData, {
        headers: {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh' }}>
      <div style={{ maxWidth: '400px', padding: '30px', backgroundColor: '#f0f0f0', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', fontFamily: 'Arial, sans-serif', textAlign: 'center' }}>
        <h1 style={{ fontSize: '24px', marginBottom: '30px', color: '#022019' }}>Waste Classification using Images</h1>
        <button
          style={{ 
            width: '100%', 
            padding: '12px',
        ...
        <label htmlFor="fileInput" style={{ marginBottom: '20px', width: '100%', cursor: 'pointer', display: 'block' }}>
          <button onClick={handleChooseFile} style={{ width: '100%',padding: '12px', fontSize: '16px', backgroundColor: '#36b02f', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', transition: 'background-color 0.3s' }}>Choose File</button>
        </label>
        <button
